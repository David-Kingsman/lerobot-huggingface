...
class JacobiRobot:
    def __init__(
        ...
        min_linear_vel: float = 0.00,
        min_angular_vel: float = 0.00,
        ...
        try:
            ...
            print(f"Found end-effector frame '{ee_link}' with ID: {self.ee_frame_id}")
        except RuntimeError:
            print(f"Warning: Frame '{ee_link}' not found.")
            print(f"Available frames: {[self.model.frames[i].name for i in range(self.model.nframes)]}")
            # Use the last frame as a fallback (usually the end-effector)
            self.ee_frame_id = self.model.nframes - 1
            print(f"Using frame {self.ee_frame_id}: '{self.model.frames[self.ee_frame_id].name}' as fallback.")
        ...
        self.kp_ori = 1.0  # Orientation gain (reduced from 3.0)
        self.kd = 0.05  # Damping gain
        self.damping = 1e-5  # Increased damping for stability
    ...
    def get_ee_pose(self, joint_positions_dict: dict = None) -> np.ndarray:
        ...
        # Safety check for frame ID bounds
        if self.ee_frame_id >= len(self.data.oMf):
            raise IndexError(f"End-effector frame ID {self.ee_frame_id} is out of range. "
                           f"Available frames: 0-{len(self.data.oMf)-1}")
    ...
    def _get_ee_pose_given_positions(self, joint_positions_dict: dict) -> np.ndarray:
        ...
        # Safety check for frame ID bounds
        if self.ee_frame_id >= len(self.data_tmp.oMf):
            raise IndexError(f"End-effector frame ID {self.ee_frame_id} is out of range. "
                           f"Available frames: 0-{len(self.data_tmp.oMf)-1}")
